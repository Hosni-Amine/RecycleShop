                        ...
                        <table className="table">
                          <thead>
                            <tr>
                              ...
                              <th className="center-cell" >Order status</th>
                          ...
                          <tbody>
                            {ordersData.map((order) => (
                              <tr key={order.id}>
                                ...
                                {order.created_at ? formatDate(order.created_at) : 'Loading...'}
                                </td>
                                ...
                                <td className="center-cell"style={{ backgroundColor: order.status === 1 ? 'lightgreen' : 'inherit' }}>
                                  {order.status === 1 ? "Approved" : order.status === 2 ? "Pending" : "On Hold"}
                                </td>
                                <td className="center-cell" style={{ backgroundColor: order.status === 1 ? 'lightgreen' : 'inherit' }}>
                                    <Link to={`/orderdetails/${order.id}`} style={{ marginLeft: 20 }}>
